...
YubiKey FIDO2認証を選択する場合は、YubiKey Managerを使用してYubiKey FIDO2 PINを設定し、Windowsの場合はPuTTY-CAC（Common Access Card）で、MacOSの場合はssh-keygenを使用してFIDO2キーを生成します。手順はテクニカルレポートに記載されています。 link:https://docs.netapp.com/us-en/ontap-technical-reports/security.html#multifactor-authentication["TR-4647: ONTAPにおける多要素認証"^] 。
...
* FIDO2公開鍵は `id_ecdsa_sk.pub`ファイルまたは `id_edd519_sk.pub` TR-4647 の 39 ページの「YubiKey FIDO2 認証用に MAC OS または Linux SSH クライアントを構成する」セクションで説明されているように、ECDSA を使用するか EDD519 を使用するかに応じて、ファイルを作成します。
...
* SSH互換形式で公開鍵をエクスポートするには、  `ssh-keygen -e` PIVのコマンド
