...
summary: 仮想NAS（vNAS）環境は、VSAN上のONTAP Selectクラスタ、一部のHCI製品、 NetApp HCIテクノロジー、および外部アレイタイプのデータストアをサポートします。これらの構成の基盤となるインフラストラクチャは、データストアの耐障害性を実現します。 
---
...
[role="lead"]
仮想NAS（vNAS）環境は、仮想SAN（VSAN）、一部のHCI製品、および外部アレイタイプのデータストア上のONTAP Selectクラスタをサポートします。これらの構成の基盤となるインフラストラクチャは、データストアの耐障害性を実現します。

最小要件は、基盤となる構成が VMware によってサポートされており、それぞれの VMware HCL に記載されていることです。
...
ONTAP Select NVRAMは VMDK です。言い換えれば、 ONTAP Select はブロックアドレス指定可能なデバイス（VMDK）上でバイトアドレス指定可能な空間（従来のNVRAM）をエミュレートします。しかし、 NVRAMのパフォーマンスはONTAP Selectノード全体のパフォーマンスにとって極めて重要です。

ハードウェア RAID コントローラを備えた DAS セットアップの場合、 NVRAM VMDK へのすべての書き込みは最初にNVRAMキャッシュとして機能します。
...
ONTAP Deployは、マルチノードVNASクラスタの初期導入時に、同じクラスタから複数のONTAP Selectインスタンスが同じホストに配置されないようにします。次の図は、2つのホストにまたがる2つの4ノードクラスタの正しい導入例を示しています。
